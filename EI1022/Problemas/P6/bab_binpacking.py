...
def binpacking_solve(objects: List[int], capacity: int):
    class BinPackingBabPS(BabPartialSolution):
        ...
        def calc_opt_bound(self) -> Union[int, float]:
            max_hueco = 0
            suma_objs_caben = suma_objs_no_caben = 0;
            ...
            for num_obj in range(self.n, len(objects)):
                if objects[num_obj] <= max_hueco:
                    ...
                    suma_objs_no_caben += objects[num_obj]

            for peso_contenedor in self.container_weights:
                espacio_libre_contenedor = capacity - peso_contenedor
                if espacio_libre_contenedor >= objects[-1]:
                    suma_objs_caben -= min(suma_objs_caben, espacio_libre_contenedor)

            return len(self.container_weights) + ceil((suma_objs_no_caben + suma_objs_caben)/capacity)
        ...
        def calc_pes_bound(self) -> Union[int, float]:
            container_weights = list(self.container_weights) + [0]*(len(objects)-self.n)

            for num_obj in range(self.n, len(objects)):
                for num_container in range(len(self.container_weights)):
                    if objects[num_obj] <= container_weights[num_container]:
                        container_weights[num_container] -= objects[num_obj]
                        break
