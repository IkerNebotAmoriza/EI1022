...
def vacias(s: Sudoku) -> Optional[Position]:
    return [(f, c) for f in range(9) for c in range(9) if s[f][c] == 0]
...
class SudokuPS(PartialSolution):
    ...
    def get_solution(self) -> Sudoku:
        if not self.is_solution(): raise RuntimeError("No es solucion")
        return self.s

    # Devuelve la lista de sus sol. parciales sucesoras
    def successors(self) -> Iterable["SudokuPS"]:
        pos = argmin(self.vacias, lambda pos: len(posibles_en(self.s, pos[0], pos[1])))
